module InwardRemittanceHelper
  def find_inward_remittances(params)
    ...
    inw_remittances = inw_remittances.where("lower(partner_code)=?",params[:partner_code].downcase) if params[:partner_code].present?
    ...
    inw_remittances = inw_remittances.where("transfer_ccy>=? and transfer_ccy <=?",params[:from_amount].to_f,params[:to_amount].to_f) if params[:to_amount].present? and params[:from_amount].present?
