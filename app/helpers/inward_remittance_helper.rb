module InwardRemittanceHelper
  def find_inward_remittances(params)
    ...
    inw_remittances = inw_remittances.where("transfer_ccy>=? and transfer_ccy <=?",params[:from_amount],param[:to_amount]) if params[:to_amount].present? and params[:from_amount].present
