module IcInvoicesHelper
  def find_ic_invoices(params)
    ...
    ic_invoices = ic_invoices.where("repaid_amount>=? and repaid_amount<=?",params[:repaid_from_amount].to_f,params[:repaid_to_amount].to_f) if params[:repaid_to_amount].present? and params[:repaid_from_amount].present?
