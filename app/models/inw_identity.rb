class InwIdentity < ActiveRecord::Base
  ...
  validates_presence_of :partner_id, :created_by, :updated_by

  def whitelisted_identity
    WhitelistedIdentity.where("full_name =? and id_type=? and id_number=? and id_country=? and id_issue_date=? and id_expiry_date=? and is_verified=?", full_name,id_type,id_number,id_country,id_issue_date,id_expiry_date,'Y').first rescue nil
  end
