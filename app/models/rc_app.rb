class RcApp < ActiveRecord::Base
  ...
  def encrypt_password
    self.http_password = EncPassGenerator.new(self.http_password, ENV['CONSUMER_KEY'], ENV['CONSUMER_SECRET']).generate_encrypted_password unless self.http_password.to_s.empty?
  end

  def self.decrypted_value(data)
    DecPassGenerator.new(data,ENV['CONSUMER_KEY'], ENV['CONSUMER_SECRET']).generate_decrypted_data unless data.nil?
  end
