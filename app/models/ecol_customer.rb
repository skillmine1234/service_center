class EcolCustomer < ActiveRecord::Base  
  ...
  validates :app_code, format: {with: /\A[a-z|A-Z|0-9]+\z/, :message => 'Invalid format, expected format is : {[a-z|A-Z|0-9]}' }, length: {maximum: 15}
  ...
  validates :debit_acct_val_fail, format: {with: /\A[0-9]+\z/, :message => 'Invalid format, expected format is : {[0-9]}' }, length: {maximum: 15, minimum: 15}, :allow_blank => true
  ...
  def customer_code_format
    if !code.nil? && code.start_with?("9")
      unless code =~ /\A(9[0-9]{3})\Z/i
        errors.add(:code, "the code can be either a 4 digit number starting with 9, or a 8 character alpha-numeric code, that does not start with 9")
      end
    else
      unless code =~ /\A[0-9A-Za-z]{8}\Z/i
        errors.add(:code, "the code can be either a 4 digit number starting with 9, or a 8 character alpha-numeric code, that does not start with 9")
      end
    end
  end
