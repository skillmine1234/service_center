class PcProduct < ActiveRecord::Base
  ...
  validates :card_cust_id, :card_acct, :sc_gl_income, format: {with: /\A[a-z|A-Z|0-9]+\z/, :message => "Invalid format, expected format is : {[a-z|A-Z|0-9]}" }
  ...
  validates :card_cust_id, length: { minimum: 3, maximum: 10 }
  ...
  def self.options_for_pc_products
    PcProduct.all.collect { |pc_product| [pc_product.code, pc_product.code] }
  end
