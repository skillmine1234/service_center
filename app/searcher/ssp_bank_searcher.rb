class SspBankSearcher
  ...
  def paginate
    if valid? 
      ...
      SspBank.none.paginate(per_page: PER_PAGE, page: page)
    end
  end
  ...
  def find
    reln = approval_status == 'U' ? SspBank.unscoped.where("approval_status =?",'U').order("id desc") : SspBank.order("id desc")
