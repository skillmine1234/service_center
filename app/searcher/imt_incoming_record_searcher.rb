class ImtIncomingRecordSearcher
  ...
  def find
    ...
    reln = reln.where("imt_incoming_records.transfer_amount>=? and imt_incoming_records.transfer_amount<=?", from_amount.to_f,to_amount.to_f) if from_amount.present? and to_amount.present?
    reln
  end
end