class CreateSmAuditLogs < ActiveRecord::Migration[7.0]
  def change
    create_table :sm_audit_logs do |t|#, {:sequence_start_value => '1 cache 20 order increment by 1'} do |t|
      ...
      t.string :sm_auditable_type, :limit => 255, :null => false, :comment => "the name of the table that represents the request that is related to this record"
      t.integer :sm_auditable_id, :null => false, :comment => "the id of the row that represents the request that is related to this recrod"
      ...
      t.index([:app_id, :req_no, :attempt_no], :unique => true, :name => 'uk_sm_audit_logs_1')
      t.index([:sm_auditable_type, :sm_auditable_id], :unique => true, :name => 'uk_sm_audit_logs_2')      
    end
  end
end