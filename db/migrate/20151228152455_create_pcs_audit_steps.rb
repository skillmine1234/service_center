class CreatePcsAuditSteps < ActiveRecord::Migration
  def change
    create_table :pcs_audit_steps, {:sequence_start_value => '1 cache 20 order increment by 1'} do |t|
      t.string :pcs_auditable_type, :null => false, :comment => "the name of the table that represents the request that is related to this record"
      ...
      t.index([:pcs_auditable_type, :pcs_auditable_id, :step_no, :attempt_no], :unique => true, :name => "uk_pcs_audit_steps")
    end
  end
end
