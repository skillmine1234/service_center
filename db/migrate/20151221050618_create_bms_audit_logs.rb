class CreateBmsAuditLogs < ActiveRecord::Migration
  def change
    create_table :bms_audit_logs do |t|
      ...
      t.string :bms_auditable_type, :limit => 255, :null => false, :comment => "the name of the table that represents the request that is related to this record"
      t.integer :bms_auditable_id, :null => false, :comment => "the id of the row that represents the request that is related to this recrod"
      ...
      t.index([:app_id, :req_no, :attempt_no], :unique => true, :name => 'uk_bms_audit_logs_1')
      t.index([:bms_auditable_type, :bms_auditable_id], :unique => true, :name => 'uk_bms_audit_logs_2')
    end
  end
end
