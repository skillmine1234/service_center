class CreatePc2UnLoadCard < ActiveRecord::Migration
  def change
    create_table :pc2_unload_cards do |t|
      ...
      t.string :credit_acct_no, :null => false, :limit => 50, :comment => "the account that needs to be debited"
      ...
      t.string :unload_ccy, :null => false, :limit => 50, :comment => "the currency code of the transaction. Only INR transactions are accepted."
      t.number :unload_amount, :null => false, :comment => "the transaction amount, represented in the transaction currency."
      ...
      t.index([:req_no, :app_id, :attempt_no], :unique => true, :name => 'uk_pc2_unload_cards')            
    end
  end
end
