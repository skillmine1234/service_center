class CreateSspBanks < ActiveRecord::Migration
  def change
    create_table :ssp_banks, {:sequence_start_value => '1 cache 20 order increment by 1'} do |t|
      t.string :customer_code, limit: 10, comment: 'the unique code for the ssp_bank'
      ...
      t.string :get_status_url, limit: 100, comment: 'status url'      
      t.string :get_account_status_url, limit: 100, comment: 'account status url'      
      t.string :http_username, limit: 100, comment: 'the http_username for the ssp_bank'
      t.string :http_password, limit: 255, comment: 'the http_password for the ssp_bank'
      t.integer :settings_cnt, comment: 'the count of settings for the ssp_bank'
      t.string :setting1, comment: 'the setting 1 for the ssp_bank'
      t.string :setting2, comment: 'the setting 2 for the ssp_bank'
      t.string :setting3, comment: 'the setting 3 for the ssp_bank'
      t.string :setting4, comment: 'the setting 4 for the ssp_bank'
      t.string :setting5, comment: 'the setting 5 for the ssp_bank'
      ...
      t.string :app_code, limit: 50, null: false, comment: 'the app_code for the ssp_bank'
      t.integer :max_retries_for_notify, comment: 'the maximum no. of retries for notification for the ssp_bank'
      t.integer :retry_notify_in_mins, comment: 'the interval in minutes for retrying notification for the ssp_bank'
