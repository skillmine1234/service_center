class CreatePc2AuditLog < ActiveRecord::Migration
  def change
    create_table :pc2_audit_logs do |t|
      ...
      t.string :pc2_auditable_type, :limit => 255, :null => false, :comment => "the name of the table that represents the request that is related to this record"
      t.integer :pc2_auditable_id, :null => false, :comment => "the id of the row that represents the request that is related to this recrod"
      ...
      t.index([:app_id, :req_no, :attempt_no], :unique => true, :name => 'uk_pc2_audit_logs_1')
      t.index([:pc2_auditable_type, :pc2_auditable_id], :unique => true, :name => 'uk_pc2_audit_logs_2')
    end
  end
end
