class CreateTableFmOutputFiles < ActiveRecord::Migration
  def change
    create_table :fm_output_files , {:sequence_start_value => '1 cache 20 order increment by 1'} do |t|
      ...
      t.string :step_name, limit: 50, null: false, comment: 'the step name which specify the output file'
      t.string :status_code, limit: 50, null: false, comment: 'the status code of the output file'            
      ...
      t.index([:incoming_file_id, :step_name, :status_code], unique: true, name: 'fm_output_files_01')
    end    
  end
end
