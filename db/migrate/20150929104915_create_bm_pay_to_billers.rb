class CreateBmPayToBillers < ActiveRecord::Migration
  def change
    create_table :bm_pay_to_billers do |t|
      ...
      t.float :bill_amount, :comment => 'the bill amount, required if payment is towards a bill'
      t.float :payment_amount, :comment => 'the amount to be paid, requirement if payment is without a bill'
