class CreatePcBlockCards < ActiveRecord::Migration
  def change
    create_table :pc_block_cards do |t|
      ...
      t.string :email_id, :limit => 255, :comment => "the emai id of the customer which is derived by customer's mobile no"
      t.string :cust_uid, :limit => 255, :comment => "the unique no of the customer"
      t.string :password, :limit => 255, :comment => "the password of the customer which is derived by customer's mobile no"
