class CreateBmAuditSteps < ActiveRecord::Migration
  def change
    create_table :bm_audit_steps, {:sequence_start_value => '1 cache 20 order increment by 1'} do |t|
      t.string :bm_auditable_type, :null => false, :comment => "the name of the table that represents the request that is related to this record"
      ...
      t.index([:bm_auditable_type, :bm_auditable_id, :step_no, :attempt_no], :unique => true, :name => "bm_audit_steps_01")
    end
  end
end
